...
time3cyc = [5000, 7000, 13000, 7000, 11000, 1000, 1000, 300, 500, 4000, 1000, 8000, 1000, 500, 1000, 2000]
time3cur = 0
...
#           Проверяем нажатие с помощью collidepoint()
    ...
    if buttoncheck1 == 1:
        if clicked_grid:
            ...
            if curpos not in plantpos:
                if money >= 50:
                    plantpos.append(curpos)
                    pplantpos.append(purpos)
                    money -= 50
                    plants.append(plant.Plant(screen, "Sunflower", 50, clicked_grid[0], clicked_grid[1], (((clicked_grid[1])-screen_y/(875/100)) // (screen_y/(875/80))), 0))
                    text_money = button_font.render(f"Money: {money}", True, [0, 0, 0])
                    clicked_grid = None
                    buttoncheck1 = 0
    elif buttoncheck1 == 2:
        if clicked_grid:
            ...
            if curpos not in plantpos:
                if money >= 100:
                    ...
                    pplantpos.append(purpos)
                    money -= 100
                    ff = plant.Plant(screen, "Peashoot", 0, clicked_grid[0], clicked_grid[1], (((clicked_grid[1])-screen_y/(875/100)) // (screen_y/(875/80))), current_time)
                    plants.append(ff)
                    text_money = button_font.render(f"Money: {money}", True, [0, 0, 0])
                    clicked_grid = None
                    if ff.row < 4.0:
        ...
        if clicked_grid:
            ...
            if curpos not in plantpos:
                if money >= 50:
                    ...
                    pplantpos.append(purpos)
                    money -= 50
                    plants.append(plant.Plant(screen, "Wallnut", 0, clicked_grid[0], clicked_grid[1], (((clicked_grid[1])-screen_y/(875/100)) // (screen_y/(875/80))), 0))
                    text_money = button_font.render(f"Money: {money}", True, [0, 0, 0])
                    clicked_grid = None
                    buttoncheck1 = 0
    elif deletecheck == 1:
    ...
    if mode == "1":
        ...
        for q in plants:
            if q.plant == "Sunflower":
                q.appear(pvz_sunflower)
            if q.plant == "Peashoot":
                q.appear(pvz_peashooter)
                for ag in zembies:
                    if ag.zrng == q.row and q.time < current_time:
                        print(f"hi from {q.row}")
                        q.time = (current_time//5000+1) * 5000 + (q.time % 5000)
                        print("bullet")
                        bullets.append([10, 1, (q.pos_x+screen_x/25), q.pos_y, q.row])
                    elif q.time < current_time:
                        q.time = (current_time//5000+1) * 5000 + (q.time % 5000)
            if q.plant == "Wallnut":
        ...
        for ah in bullets:
            ...
            for ai in zembies:
                ...
                if int(ai.pos_x) == int(ah[2]) and ah[4] == ai.zrng:
                    bullets.pop(bullets.index(ah))
            screen.blit(pvz_pea, [ah[2], (ah[3]+screen_y/70)])
            ah[2] += ah[1] * screen_y/(875/1.1)
        if time4 < current_time:
            time4 += 10000
            sunflowersuncheck = 1
            for ps in pplantpos:
                if ps[0] == "S":
                    pss = entity.Sun(int(ps[1:4]), int(ps[4:]), screen_x, screen_y)
        ...
        if time5 < current_time:
            sunflowerlist = []
            time5 += 10000
        if sunflowersuncheck == 1:
            for ps in sunflowerlist:
                ps.sunappear(screen)
